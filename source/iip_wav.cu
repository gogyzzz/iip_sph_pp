...
printf("%s\n",__func__);
  ...
  mat->ndim=1;
	...
	mat->d1 = buf->channels;
  mat->d2 = 1;
	con =(DTYPE*)malloc(sizeof(DTYPE)*mat->d0 * mat->d1);
  cudaMalloc((void**)&(mat->data),sizeof(DTYPE)*mat->d0 * mat-> d1);  
  
	for(i=0;i<mat->d1; i++)
	{
			for(j=0;j<mat->d0;j++)
			{
				con[i * mat->d0 + j ] = (DTYPE)(buf->buf[i + mat->d1*j  ]) ;
			}			
	}
  cudaMemcpy(mat->data,con,sizeof(DTYPE)*mat->d0 * mat-> d1, cudaMemcpyHostToDevice);
  free(con);
return mat;
}
...
printf("%s\n",__func__);
  ...
  wav->riff_size = wav->data_size + 44;
	...
	con = (DTYPE*)malloc(sizeof(DTYPE)*d0*d1);
  cudaMemcpy(con,mat->data,sizeof(DTYPE)*d0*d1,cudaMemcpyDeviceToHost);	

	wav->buffer.buf = (short*)malloc(sizeof(short)*d0*d1);
	for(i=0;i<d0*d1;i++)
	{
		wav->buffer.buf[i] = (short)(con[i%d1*d0 + i/d1]);
	}
