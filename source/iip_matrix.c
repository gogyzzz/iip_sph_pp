...
#pragma omp parallel for schedule(dynamic) shared(submat, mat) private(i, j, k)
...
DIM *alloc_dim_0d() { return alloc_dim_3d(1, 1, 1); }
DIM *alloc_dim_1d(UINT d0) { return alloc_dim_3d(d0, 1, 1); }
DIM *alloc_dim_2d(UINT d0, UINT d1) { return alloc_dim_3d(d0, d1, 1); }
...
void add_elements(MAT *A, MAT *B, MAT *C) {
  ...
  /*1 to 1*/
  if (a2 == b2) {
    if (c2 != a2) ASSERT_DIM_INVALID()
    ia = a0 * a1;
    ib = b0 * b1;
  }
  /*1 to b2*/
  else if (a2 == 1 && b2 != 1) {
    if (b2 != c2) ASSERT_DIM_INVALID()
    ia = 0;
    ib = b0 * b1;
  }
  /* 1 to a2 */
  else if (a2 != 1 && b2 == 1) {
    if (a2 != c2) ASSERT_DIM_INVALID()
    ib = 0;
    ia = a0 * a1;
  }
  /* unmatching*/
  ...
  if (((a0 == 1) && (a1 == 1)) && ((b0 == 1) && (b1 != 1))) {
    if ((C->d0 != 1) || (C->d1 != b1)) ASSERT_DIM_INVALID()
    for (j = 0; j < c2; j++)
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < b1; i++) {
...
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
  ...
  // 1 1 b0 b1
    ...
    if ((C->d0 != b0) || (C->d1 != b1)) ASSERT_DIM_INVALID()
    for (j = 0; j < c2; j++)
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < b0 * b1; i++) {
        C->data[ic * j + i] = A->data[ia * j + 0] + B->data[ib * j + i];
  ...
  // a0 a1 1 1
    ...
    if ((C->d0 != a0) || (C->d1 != a1)) ASSERT_DIM_INVALID()
    for (j = 0; j < c2; j++)
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < a0 * a1; i++) {
        C->data[ic * j + i] = A->data[ia * j + i] + B->data[ib * j + 0];
  ...
  // 1 a1 b0 1
    ...
    if ((C->d0 != b0) || (C->d1 != a1)) ASSERT_DIM_INVALID()
    for (j = 0; j < c2; j++)
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < b0 * a1; i++) {
        C->data[ic * j + i] =
...
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
  ...
  // 1  a1 b0 b1 | b0 a1 (a1==b1)
    ...
    if (((C->d0 != b0) || (C->d1 != a1)) || (a1 != b1)) ASSERT_DIM_INVALID()
    for (j = 0; j < c2; j++)
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < b0 * a1; i++) {
  ...
  // a0 1  b0 b1 | b0 b1 (a0==b0)
    ...
    if (((C->d0 != b0) || (C->d1 != b1)) || (a0 != b0)) ASSERT_DIM_INVALID()
    for (j = 0; j < c2; j++)
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < b0 * b1; i++) {
  ...
  // a0 a1 b0  1 | b0 a1 (a0==b0)
    ...
    if (((C->d0 != b0) || (C->d1 != a1)) || (a0 != b0)) ASSERT_DIM_INVALID()
    for (j = 0; j < c2; j++)
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < b0 * a1; i++) {
  ...
  // a0 a1 1  b1 | a0 b1 (a1==b1)
    ...
    if (((C->d0 != a0) || (C->d1 != b1)) || (a1 != b1)) ASSERT_DIM_INVALID()
    for (j = 0; j < c2; j++)
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < a0 * b1; i++) {
...
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < b0 * a1; i++) {
        ASSERT(B->data[ib * j + i % b0], "Divide by zero.\n")
        C->data[ic * j + i] =
            A->data[ia * j + i / a1] / B->data[ib * j + i % b0];
      }
  }
...
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < a0 * b1; i++) {
        ASSERT(B->data[ib * j + i / b1], "Divide by zero.\n")
        C->data[ic * j + i] =
            A->data[ia * j + i % a0] / B->data[ib * j + i / b1];
      }
  }
...
#pragma omp parallel for schedule(dynamic) shared(C, B, A) private(i)
      for (i = 0; i < b0 * a1; i++) {
        ASSERT(B->data[ib * j + i % a1], "Divide by zero.\n")
        C->data[ic * j + i] = A->data[ia * j + i] / B->data[ib * j + i % a1];
      }
  }
  // a0 a1 1  b1 | a0 b1 (a1==b1)
...
#pragma omp parallel for schedule(dynamic) shared(X) private(i)
  for (i = 0; i < size; i += incx) {
    ASSERT(X[i].re != 0 || X[i].im != 0, "Divide by zero.\n")
...
#pragma omp parallel for schedule(dynamic) shared(mat, t_mat) private(i, l, k, \
                                                                      j)
  for (i = 0; i < size; i++) {
    for (l = 0; l < dim->d2; l++)
      for (k = 0; k < dim->d1; k++)
...
void permute(MAT *mat, UINT seq) {
  ...
  if (seq == 132) {
    ...
    mat->d1 = t->d2;
    mat->d2 = t->d1;
#pragma omp parallel for schedule(dynamic) shared(mat, t) private(i)
...
#pragma omp parallel for schedule(dynamic) shared(t_mat, mat) private(i)
    for (i = 0; i < d0; i++) {
...
#pragma omp parallel for schedule(dynamic) shared(t_mat, mat) private(i, j)
    for (j = 0; j < d2; j++) {
      for (i = 0; i < d0 * d1; i++) {
...
void transpose(MAT *mat) {
  ...
  if (mat->ndim == 0) {
#pragma omp parallel for schedule(dynamic) shared(temp, mat) private(i)
    for (i = 0; i < d0; i++) {
...
#pragma omp parallel for schedule(dynamic) shared(temp, mat) private(i, j)
    for (j = 0; j < d2; j++) {
      for (i = 0; i < d0 * d1; i++) {
...
void diagonal(MAT *mat, MAT *dia) {
  ...
  if (mat->d2 != dia->d2) ASSERT_DIM_INVALID()
  if (dia->d1 != 1) ASSERT_DIM_INVALID()
  if (mat->d0 != dia->d0) ASSERT_DIM_INVALID()
...
void trace(MAT *mat, MAT *tr) {
  ...
  if (mat->d2 != tr->d2) ASSERT_DIM_INVALID()
  if (tr->d1 != 1) ASSERT_DIM_INVALID()
  if (tr->d0 != 1) ASSERT_DIM_INVALID()
  ASSERT((mat->d0 == mat->d1), "This function requires SQUARE MATRIX.\n");
...
#pragma omp parallel for schedule(dynamic) shared(mat) private(i, j)
    for (i = 0; i < d0; i++) {
      for (j = 0; j < d0; j++) {
        if (i == j) {
          mat->data[i * d0 + j] = 1;
    ...
    d0 = mat->d0;
    d2 = mat->d2;
#pragma omp parallel for schedule(dynamic) shared(mat) private(i, j, k)
    for (i = 0; i < d2; i++) {
      for (j = 0; j < d0; j++) {
        for (k = 0; k < d0; k++) {
          if (i == j) {
...
#pragma omp parallel for schedule(dynamic) shared(mat) private(i, j, k)
    for (i = 0; i < d2; i++) {
      for (j = 0; j < d0; j++) {
        for (k = 0; k < d0; k++) {
          if (i == j) {
            mat->data[i * d0 * d0 + j * d0 + k].re = 1;
            mat->data[i * d0 * d0 + j * d0 + k].im = 0;
          } else {
            mat->data[i * d0 * d0 + j * d0 + k].re = 0;
            mat->data[i * d0 * d0 + j * d0 + k].im = 0;
          }
        }
      }
