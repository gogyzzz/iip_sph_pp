...
WAV *read_wav(char *file_path) {
  ...
  printf("Buffer read : %lu\n", fread(wav->buffer.buf, sizeof(short),
                                      wav->channels * num_of_sample, f));
...
void write_wav(WAV *wav, char *file_path) {
  ...
  printf("WRITE BUFFER : %lu\n",
         fwrite((wav->buffer.buf), sizeof(short),
                wav->data_size / (wav->bit_per_sample / 8), f));
...
void print_wav(WAV *wav) {
  ...
  printf("riff_id        : %c%c%c%c\n", wav->riff_id[0], wav->riff_id[1],
         wav->riff_id[2], wav->riff_id[3]);
  printf("riff_size      : %u \n", wav->riff_size);
  printf("wave_id        : %c%c%c%c\n", wav->wave_id[0], wav->wave_id[1],
         wav->wave_id[2], wav->wave_id[3]);
  printf("fmt_id         : %c%c%c%c\n", wav->fmt_id[0], wav->fmt_id[1],
         wav->fmt_id[2], wav->fmt_id[3]);
  printf("fmt_size       : %u\n", wav->fmt_size);
  t = wav->fmt_type;
  switch (t) {
    case 1:
  ...
  printf("data_id        : %c%c%c%c\n", wav->data_id[0], wav->data_id[1],
         wav->data_id[2], wav->data_id[3]);
  printf("data_size      : %u\n", wav->data_size);
}
...
#pragma omp parallel for schedule(dynamic,CHUNK_SIZE) shared(mat, buf) private(i, j)
  for (i = 0; i < mat->d1; i++) {
    for (j = 0; j < mat->d0; j++) {
...
#pragma omp parallel for schedule(dynamic,CHUNK_SIZE) shared(wav, mat) private(i)
  for (i = 0; i < d0 * d1; i++) {
