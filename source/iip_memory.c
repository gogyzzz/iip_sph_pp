...
void init() {
  ...
  // memory_log[0] = (USED_MEM*)malloc(sizeof(USED_MEM) * (MEM_PAGE_BASIC_SIZE /
  // LOG_ALLOC_UNIT));
  // memset(memory_log[0], 0, sizeof(USED_MEM) * (MEM_PAGE_BASIC_SIZE /
  // LOG_ALLOC_UNIT));
...
void* iip_malloc(unsigned long int size) {
  ...
  for (i = 0; i < pool_cnt; i++) {
    ...
    if (alloc_idx != -1) {
      ...
      printf("return %lu[%lu] + %ld\n", (unsigned long int)memory_pool[i], i,
             alloc_idx);
#endif
  ...
  do {
    ...
    // memory_log[pool_cnt] = (USED_MEM*)malloc(sizeof(USED_MEM) * (page_size /
    // LOG_ALLOC_UNIT));
    // memset(memory_log[pool_cnt], 0, sizeof(USED_MEM) * (page_size /
    // LOG_ALLOC_UNIT));
  ...
  if (memory_pool[pool_cnt - 1] != NULL) {
    ...
    log_cnt[pool_cnt - 1]++;
#if DEBUG
    printf("return %lu[%lu]\n", (unsigned long int)memory_pool[pool_cnt - 1],
           pool_cnt - 1);
#endif
...
void iip_free(void* ptr) {
  ...
  for (i = 0; i < pool_cnt; i++) {
    for (j = 0; j < log_cnt[i]; j++) {
      // Find!
      if (ptr ==
          (unsigned long int)memory_pool[i] + memory_log[i][j].frag_idx) {
...
signed long int page_alloc_isable(int page_idx,
                                  unsigned long int require_size) {
