...
/*┌─────────────────────────────────────┐
  │                    Fast Fourier Transform (MATLAB Style)                 │
  └─────────────────────────────────────┘*/
/* Takuya Ooura's FFT */
void fft(double *a, int halfn) {
...
/*┌─────────────────────────────────────┐
  │               Inverse Fast Fourier Transform (MATLAB Style)              │
  └─────────────────────────────────────┘*/
/* Takuya Ooura's FFT */
void ifft(double *a, int halfn) {
...
double **zeros2(int rows, int cols) {
  ...
  *i = rows;
  ...
  for (j = 1; j <= rows; j++) v[j] = zeros(cols);
...
double ***zeros3(int rows, int cols, int depth) {
  double **zeros2(int rows, int cols);
  double ***v;
  int *i, j;
  ...
  *i = rows;
  ...
  for (j = 1; j <= rows; j++) v[j] = zeros2(cols, depth);

  return v;
}

void free3(double ***v) {
  void size3(double ***v, int *rows, int *cols, int *depth);
  int rows, cols, depth, i, j;

  size3(v, &rows, &cols, &depth);
  for (i = 1; i <= rows; i++) {
    for (j = 1; j <= cols; j++) free(v[i][j]);
    free(v[i]);
  }
  free(v);
}
...
float **zeros2f(int rows, int cols) {
  float *zerosf(int len);
  float **v;
  int *i, j;

  v = (float **)calloc((rows + 1), sizeof(float *));
  i = (int *)v;
  *i = rows;

  for (j = 1; j <= rows; j++) v[j] = zerosf(cols);

  return v;
}

void free2f(float **v) {
  void sizef(float **v, int *rows, int *cols);
  int rows, cols, i;
...
float ***zeros3f(int rows, int cols, int depth) {
  float **zeros2f(int rows, int cols);
  float ***v;
  int *i, j;
  ...
  *i = rows;
  ...
  for (j = 1; j <= rows; j++) v[j] = zeros2f(cols, depth);

  return v;
}

void free3f(float ***v) {
  void size3f(float ***v, int *rows, int *cols, int *depth);
  int rows, cols, depth, i, j;

  size3f(v, &rows, &cols, &depth);
  for (i = 1; i <= rows; i++) {
    for (j = 1; j <= cols; j++) free(v[i][j]);
    free(v[i]);
  }
...
int *CreateIntVec(int size) {
  int *v;

  v = (int *)malloc((size + 1) * sizeof(int));
  *v = size;
  return v;
}

int IntVecSize(int *v) { return *v; }

/* EXPORT->ZeroIntVec: Zero the elements of v */
void ZeroIntVec(int *v) {
  int i, n;
...
void cftmdl2(int n, double *a) {
  int i, i0, j, j0, j1, j2, j3, m, mh;
  double ew, w1r, w1i, wn4r, wk1r, wk1i, wk3r, wk3i, wl1r, wl1i, wl3r, wl3i,
